...
async def join_tournament(
    ...
    if not await db_actions.join_tournament(
        ...
        raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail="Ви не можете приєднатися до турніру")
...
async def add_result(
    ...
    if not await db_actions.add_result_by_team(
        ...
        raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail="Ви не можете додати результат")
...
async def add_vote(
    ...
    if not await db_actions.add_vote(
        **vote_model.model_dump(),
        ...
        raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail="Ви не можете проголосувати")
...
async def check_vote(
    ...
    if result is None:
        raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE, detail="Ви не можете перевірити результат голосування")
    
    if result:
        return dict(message="Голосування пройшло успішно, більшість вирішала приймати участь у турнірі")
    else:
        return dict(message="Голосування пройшло успішно, більшість вирішала не приймати участь у турнірі")
...
@tournaments_router.get("/", status_code=status.HTTP_200_OK, response_model=List[TournamentModelResponse])
